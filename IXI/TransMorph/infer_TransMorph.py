...
def main():
    ...
    if running_on_dgx:
        ...
        checkpoint_path = '/data/users/adarc/registration/registration-remote/output/experiments/' \
                          'scheduler0.001_prev_was_0.01_IXI_TransMorph_ncc_1_diffusion_1/best.pth.tar'
    else:
    ...
    model_idx = -1
    weights = [1, 1]
    process_name = f'IXI_TransMorph_ncc_{weights[0]}_diffusion_{weights[1]}'

    model_dir = 'experiments/' + process_name
    if 'Val' in test_dir:
        csv_name = process_name+'_Val'
    else:
        csv_name = process_name
    ...
    csv_writter(process_name, 'Quantitative_Results/' + csv_name)
    line = ''
    for i in range(46):
        line = line + ',' + dict[i]
    ...
    model.to(device)
    reg_model = utils.register_model(device, config.img_size, 'bilinear')
    reg_model.to(device)
    test_composed = transforms.Compose([trans.Seg_norm(),
                                        trans.NumpyType((np.float32, np.int16)),
                                        ])
    test_set = datasets.IXIBrainInferDataset(glob.glob(test_dir / '*.pkl').as_posix(), atlas_dir, transforms=test_composed)
